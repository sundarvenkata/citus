...
SELECT create_reference_table('users_reference_table');
 ...
 -- 
 -- Should error out, there exist a reference table as a part of UNION
 --
...
-- Should error out, since first subquery of UNION has a reference table
SELECT ("final_query"."event_types") as types, count(*) AS sumOfEventType
FROM
  ( SELECT *, random()
   FROM
...
-- UNION ALL query with reference table should also error out
SELECT ("final_query"."event_types") as types, count(*) AS sumOfEventType
FROM
  ( SELECT *, random()
   FROM
...
-- union all with inner and left joins should error out, since there is a reference 
-- in the left part of the left outer join
...
-- same query with subuqery joins in topmost select with reference table
        ...
        (SELECT 
         ...
         INNER JOIN
           (SELECT 
              "users"."user_id"
            FROM 
              users_referenece_table as "users"
            WHERE 
